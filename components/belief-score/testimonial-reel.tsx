 ...
 * sound and pauses whichever other clip was playing. preload="none" + poster
 * frames keep the 12 clips almost free until interaction. Styled in the marine
 * design system to match the rest of the page.
 */
const CLIP_COUNT = 12
...
export function TestimonialReel() {
  ...
  return (
    <div className="relative">
      <div
        ref={scrollerRef}
        className="-mx-6 flex snap-x snap-mandatory gap-4 overflow-x-auto px-6 pb-2 sm:-mx-10 sm:px-10 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        aria-label="Participant video testimonials"
      >
        {CLIPS.map((clip) => (
          <figure
            key={clip.id}
            className="relative w-52 shrink-0 snap-start overflow-hidden rounded-2xl border border-border bg-card/60 sm:w-60"
          >
            <video
              ref={(el) => {
            ...
            {activeId !== clip.id && (
              <button
                ...
                className="group absolute inset-0 flex cursor-pointer items-end justify-start bg-gradient-to-t from-background/60 via-transparent to-transparent p-4"
                aria-label={`Play participant testimonial video ${clip.id}`}
              >
                <span className="flex h-11 w-11 items-center justify-center rounded-full bg-signal text-background shadow-lg transition-transform duration-200 group-hover:scale-105">
      ...
      <div className="mt-5 hidden items-center justify-center gap-3 md:flex">
        <button
          ...
          className="flex h-11 w-11 cursor-pointer items-center justify-center rounded-full border border-border text-foreground transition-colors hover:bg-secondary/60"
